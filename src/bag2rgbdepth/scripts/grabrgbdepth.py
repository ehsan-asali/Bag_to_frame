...
#from sawyer_irl_project import OBlobs
...
def grabrgb(msg):
    ...
    if i_rgb <= i_depth :
        ...
        if first_flag_rgb == True :
            ...
            np.save(rgb_folder+"/frame"+str(i_rgb)+".npy",cv2.cvtColor(image_normal, cv2.COLOR_BGR2RGB))
            cv2.imwrite(rgb_folder+"/frame"+str(i_rgb)+".jpg", cv2.cvtColor(image_normal, cv2.COLOR_BGR2RGB))
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("/camera/aligned_depth_to_color/image_raw",Image,checkdepth)
    #rospy.Subscriber("/object_location",OBlobs,checkobjectlocation)

    #rospy.Subscriber("/kinect2/hd/image_color",Image,grabrgb)
    #rospy.Subscriber("/kinect2/hd/image_depth_rect",Image,checkdepth)
